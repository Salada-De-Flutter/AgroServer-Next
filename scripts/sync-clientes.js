...
async function sincronizarClientes() {
  ...
  console.log('========================================');
  console.log('SINCRONIZACAO DE CLIENTES ASAAS');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log('[2/3] Iniciando sincronizacao com Asaas...');

    while (hasMore) {
      const dadosAsaas = await listarClientesAsaas({ offset, limit });
      
      console.log(`[INFO] Processando lote: offset ${offset}, total recebido: ${dadosAsaas.data.length}`);
    ...
    console.log(`Novos clientes:      ${totalNovos}`);
    ...
    console.error('\n[ERRO] Falha na sincronizacao:', error.message);
    if (error.response) {
